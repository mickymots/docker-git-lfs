...
<body onload="init();"> 
    ...
    <table >
        ...
        <div align="center" class="slidecontainer">
            ...
            <!-- Selected Characters  -->   
            <table  >
                <tr id="selectedChars" style="display:none;"></tr>
            </table>
    ...
    <h6> Re-select Character to remove</h6>
    ...
    <script>
        ...
        function init()
        {
            ...
            document.getElementById('shark0').click();
            document.getElementById('seaturtle0').click();
    ...
    function evalSA(num)
    {
        ...
        $.ajax({
            type: "GET",
            url: "http://127.0.0.1:5000/login?diag="+val,
            crossDomain : true,
    ...
    function select_img(val,truth_val,emo_val)
        {
            ...
            if(!flag && old_flip==null && old_emo==null && images.length!=num.options[num.selectedIndex].value && old_SAbutton==null)// && old_SAtext==null)
                {
                    
                //button to mirror character vertically
                flip_td=document.createElement("td");
                flip_td.setAttribute("id","td"+prev_del);
                flip_td.setAttribute("style","padding: 6px"); 
                     
                //button to mirror character vertically
                flip_button=document.createElement("input");
                flip_button.setAttribute("type","button");
                flip_button.setAttribute("id","button"+prev_del);
                flip_button.setAttribute("value","Flip Character"+(prev_del+1));
                flip_button.setAttribute("class","1");
                flip_button.setAttribute("onclick","setup()");
                flip_button.setAttribute("style","background-color: skyblue; color: white;padding: 6px;border: none;font-size: 20px;box-shadow: 0 5px 25px rgba(0, 0, 0, 0.2);-webkit-appearance: button;appearance: button;outline: none;"); 
            ...
            if(images.length<numChars)
            {
                ...
                if(!flag)
                {
                    ...
                    selectedDiv.appendChild(flip_td);
                    flip_td.appendChild(flip_button);
...
<script src="screencap.js" type="text/javascript"></script> 
</body>
</html> 